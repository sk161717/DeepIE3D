...
def return_label(coord,center,epsilon=None):
...
def convert_sparse_to_graph(coords,center,k=None,epsilon=None):   #coords..[float,float,float]*N,input numpy
    if k==None:
        k=cfg.k
    n=len(coords)
    ...
    graph=nx.Graph()
    for i in range(n):
        graph.add_node(i,label=return_label(coords[i],center,epsilon))
    for i in range(n):
        tuple_list=list(enumerate(distance[i]))
        sort_tuple=sorted(tuple_list,key=itemgetter(1))
        D=sort_tuple[1][1]
        for j in range(1,n):
            if sort_tuple[j][1]<(1+k*cfg.f(j))*D:
