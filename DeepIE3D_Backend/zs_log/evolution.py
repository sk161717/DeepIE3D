...
class Evolution():
    ...
    def WL_evolution(self,selected_canvases,NG_canvases, zs,G,D,mode, novelty=False, behavioral=False, mutation_rate=1.0,isSimple=True):
        self.iteration+=1
        method="A" if isSimple else "B"        
        with open("log/"+self.dt+method+'log.csv','a') as f:
            writer=csv.writer(f)
            writer.writerow([mode,str(len(NG_canvases))])
        with open("log/"+self.dt+method+'itrlog.csv','w') as f:
            writer=csv.writer(f)
            writer.writerow([self.iteration])

        if (len(selected_canvases)==0 and len(NG_canvases)==0) or isSimple:
            print("Ok")
            ...
            self.generate_candidate(selected_canvases,zs,None,candidate_zs,candiate_voxels,G,mutation_rate,mode)
            ...
            self.generate_candidate(selected_canvases,zs,selected_zs,candidate_zs,candiate_voxels,G,mutation_rate,mode)
    ...
    def generate_candidate(self,selected_canvases,zs,selected_zs,candidate_zs,candidate_voxels,G,mutation_rate,mode):
...
def save_zs(zs):
    ...
    with open('zs_log/'+dt+'.csv','w') as f:
        writer=csv.writer(f)
        writer.writerows(zs)
